...
<br/><br/>
	<pre><code class="php">
$isim = htmlspecialchars(str_replace( '&lt;script>', '', $_GET[ 'isim' ] ),ENT_QUOTES);
$mesaj = htmlspecialchars(str_replace( '&lt;script>', '', $_GET[ 'mesaj' ] ),ENT_QUOTES);
	...
	<pre><code class="php">if($_POST['isim'] < 20){
		...
		<div class="col-sm-3" > 
			<form   style="margin-bottom:20px;" action="" method="post" class="form" name="form2">
				<h1 style="margin-top:10px;text-align:center;font-size:19px; font-weight:bold;">Ziyaretçi Defteri</h1>
		...
		if($isim != ''){ //isim boş değilse işlemleri yap.
				$ekle = $db->prepare( 'INSERT INTO  mesajlar (id, isim, mesaj,tarih) VALUES ( null,:isim, :mesaj, :tarih );' ); 
				$ekle->bindParam( ':isim', $isim, PDO::PARAM_STR ); 
				$ekle->bindParam( ':mesaj', $mesaj, PDO::PARAM_STR ); 
				$ekle->bindParam( ':tarih', $tarih, PDO::PARAM_STR ); 
				...
				if($ekle){
					echo'<META HTTP-EQUIV="Refresh" CONTENT="0;URL=xss-stored.php">';//Eklediyse sayfayı yeniliyoruz.
